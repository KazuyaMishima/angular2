...
describe('NuevoComponent', () => {
  ...
  it('should update the model on form submit', async(() => {
    const showButton: HTMLElement = testing.queryByCss('button');
    showButton.click();
    fixture.detectChanges();
    const importe: HTMLInputElement =  testing.queryByCss('input[name=importe]');
    const submitButton: HTMLElement = testing.queryByCss('button[name=guardar]');
    submitButton.click();
